class Connection {
    ...
    setServerUpdate(data) {
        ...
        this.player.eura.emit("debug", `[Player ${this.player.guildId} - CONNECTION] Received voice server, ${previousVoiceRegion !== null ? `Changed Voice Region from(oldRegion) ${previousVoiceRegion} to(newRegion) ${this.region}` : `Voice Server: ${this.region}`}, Updating Node's Voice Data.`)

        if (this.player.paused) {
            this.player.eura.emit(
                "debug",
    ...
    setStateUpdate(data) {
        ...
        this.player.eura.emit("debug", `[Player ${this.player.guildId} - CONNECTION] Received Voice State Update Informing the player ${channel_id !== null ? `Connected to ${this.voiceChannel}` : `Disconnected from ${this.voiceChannel}`}`)

        // If player is manually disconnected from VC
        if(channel_id == null) {
            this.player.destroy();
            this.player.eura.emit("playerDestroy", this.player);
        }
    ...
    updatePlayerVoiceData() {
        this.player.eura.emit("debug", this.player.node.name, `[Rest Manager] Sending an Update Player request with data: ${JSON.stringify({ voice: this.voice })}`)
